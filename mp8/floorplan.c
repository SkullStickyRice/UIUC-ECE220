...
*/
/*
Intro: follow a set of structs to design floorplanner.
*/
...
if (ptr->right == NULL && ptr->left == NULL && ptr->cutline == UNDEFINED_CUTLINE && ptr->parent != NULL && ptr->module != NULL)
{
return 1; /*if it's leaf node, return 1*/
}
  return 0;
}
...
{
return 1; /*if it's a internal node, return 1*/
}
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  // TODO: (remember to modify the return value appropriately)
if (a->right == b || a->left == b || b->parent == a)
{
return 1; /*special case*/
}
while (b) /*do all b values*/
{
	if(b==a)
	{
		return 1;
...
void recut(node_t* ptr) {
  ...
  // TODO: 
if(ptr->cutline == V) /*check vertical cutline*/
ptr->cutline = H; /*do recut*/
else
ptr->cutline = V; /*do recut*/
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
node_t* temp;
if((a->parent->left)==a) /*is a left or right?*/
{
	a->parent->left=b; /*replace a with b*/
}
else
{
	a->parent->right=b;
...
if((b->parent->left)==b) /*is b left or right?*/
{
	b->parent->left = a; /*replace b with a*/
}
else
{
	b->parent->right = a;
}
temp = a->parent; /*swapping*/
a->parent = b->parent;
b->parent = temp;
}
...
if(ptr->left != NULL)\
{
	postfix_traversal(ptr->left,nth,expression); /*recursion*/
}
  // Traverse the right subtree.
if(ptr->right != NULL)
{
	postfix_traversal(ptr->right,nth,expression);	/*recursion*/
}

  
  // Assign the data from the internal node.
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // Base case. Assigne the module pointer, cutline type, right child pointer, left child
